                ...
                foreach ($validationRoles as $validationRole) {
                    ...
                    if (preg_match_all('/(min)\((\d+)\)/', $validationRole, $m)) {
                        if ($this->min($value, $m[2][0]) === false) {
                            $this->messenger->add(
                                $this->_language->feedKey('text_error_' . $m[1][0], [$this->_language->get('text_label_' . $fieldName), $m[2][0]]),
                        ...
                        if ($this->between($value, $m[2][0], $m[3][0]) === false) {
                            $this->messenger->add(
                                $this->_language->feedKey('text_error_' . $m[1][0], [$this->_language->get('text_label_' . $fieldName), $m[2][0], $m[3][0]]),
                                Messenger::APP_MESSAGE_ERROR
                            );
                            $errors[$fieldName] = true;
                        }
                        ...
                        if ($this->eq_field($value, $otherFieldValue) === false) {
                            $this->messenger->add(
                                $this->_language->feedKey('text_error_' . $m[1][0], [$this->_language->get('text_label_' . $fieldName), $this->_language->get('text_label_' . $m[2][0])]),
                                Messenger::APP_MESSAGE_ERROR
                            );
                            $errors[$fieldName] = true;
                        }
                        ...
                        if ($this->$validationRole($value) === false) {
                            $this->messenger->add(
                                $this->_language->feedKey('text_error_' . $validationRole, [$this->_language->get('text_label_' . $fieldName)]),
                                Messenger::APP_MESSAGE_ERROR
                            );
                            $errors[$fieldName] = true;
                        }
